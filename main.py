...
class Board:
    ...
    def add_wall(self, a1, a2, b1, b2, players):
        ...
        if self.G.has_edge(a1, a2) and self.G.has_edge(b1, b2) and self.G.has_edge(a1, b1) and self.G.has_edge(a2,
            ...
            self.G.remove_edge(b1, a1)

            self.G.remove_edge(a2, b2)
            self.G.remove_edge(b2, a2)
...
class Game:
    ...
    def evaluate_position(self, player, players, board):
        if player.is_winner():
            return float('inf')
        score = len(player.path_find(board)) + player.walls
        for p in players:
            if p != player:
                score -= len(p.path_find(board)) + p.walls
        return score

    def offensive_wall(self, objective_player: Player, board):
    ...
    def play(self):
        ...
        while 1:
            ...
            # Primero es mover
            self.players[turn].move_pawn(self.board)
            score = self.paranoid(self.board.G, 1, (turn + 1) % len(self.players), self.players.copy())
            max_score = max(score, max_score)
            self.players[turn].current = before_current
            for i, p in enumerate(self.players):
                if i != turn:
                    if self.players[turn].wall_strategy == 1:
                        self.offensive_wall(p, self.board.G)
            ...
            if movement['type'] == 'm':
                ...
                if self.players[turn].wall_strategy == 1:
                    self.offensive_wall(self.players[movement['target']], self.board.G)
                self.players[turn].walls -= 1
    ...
    def paranoid(self, board, depth, player, players):
        if depth == 16:
            return self.evaluate_position(players[player], players, board)
        if depth % len(self.players) == 0:
            max_score = float('-inf')
            before_current = players[player].current
            before_board = board.copy()
            # Primero es mover
            players[player].move_pawn(board)
